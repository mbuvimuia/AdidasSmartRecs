...
def recommend_collaborative(user_id, n_recommendations=5):
    ...
    user_vector = user_purchases.values.reshape(1, -1)
    other_users = user_item_matrix.drop(user_id)
    similarities = cosine_similarity(user_vector, other_users)[0]
    
    # Get top similar users
    similar_users_idx = np.argsort(-similarities)[:10]  # Get top 10 similar users
    similar_users = other_users.index[similar_users_idx]
    ...
    recommendations = recommendations[user_purchases == 0]
